...
void BrightConvolver::convolve(std::vector<double> &frame1, std::vector<double> &frame2, std::vector<double> &convResult)
{
	...
	{
		convResult.push_back(frame1.at(j) * frame2.at(j) * convGain);
	}
	...
	for (int j = 0; j < fftLength; j++)
	{
		convResult.at(j) *= sqrt(sin(j*PI / fftLength));
	}
}
...
{
	// Fill previousOverlap with initial zeros
	for (int i = 0; i < 2; i++)
	{
		previousOverlap[i].clear();
		for (int j = 0; j < frameLength; j++)
		{
			previousOverlap[i].push_back(0);
		}
	}
}
...
void BrightConvolver::OnParamChange(int paramIdx)
{
	...
	{
	case kFrameLength:
		frameLength = pow(2, ((int)GetParam(kFrameLength)->Value()));
		onFrameLengthUpdate();
		break;
	case kgain:
